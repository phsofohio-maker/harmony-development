...
exports.generateDocument = onCall({
  ...
  try {
    ...
    } else if (customData.useGenericTemplate) {
      // Generic fallback template — renders patient info + merge fields
      console.log('Using generic fallback template');
      templateConfig = {
        name: documentType.replace(/_/g, ' '),
        documentType,
        layout: { pageSize: 'LETTER' },
        header: { includeOrgName: true, includeDate: true },
        sections: [
          { type: 'title', content: documentType.replace(/_/g, ' '), style: { fontSize: 16, bold: true } },
